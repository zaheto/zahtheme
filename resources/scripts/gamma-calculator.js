jQuery(document).ready(function ($) {  // Pass $ as parameter
    ...
    function calculatePrice() {
        ...
        try {
            ...
            // Price Calculations
            let totalPrice = parseFloat(gamma_pricing.base_price) * blindsProfileLm;
            //console.log('Base Price:', gamma_pricing.base_price, 'Blinds Profile Lm:', blindsProfileLm, 'Subtotal:', parseFloat(gamma_pricing.base_price) * blindsProfileLm);


           // Add essential components
            totalPrice += uProfileLeftLm * parseFloat(gamma_pricing.price_u_profile_left || 0);
            //console.log('uProfileLeftLm:', uProfileLeftLm, 'Price U Profile Left:', gamma_pricing.price_u_profile_left, 'Subtotal:', uProfileLeftLm * parseFloat(gamma_pricing.price_u_profile_left || 0));

            totalPrice += uProfileRightLm * parseFloat(gamma_pricing.price_u_profile_right || 0);
            //console.log('uProfileRightLm:', uProfileRightLm, 'Price U Profile Right:', gamma_pricing.price_u_profile_right, 'Subtotal:', uProfileRightLm * parseFloat(gamma_pricing.price_u_profile_right || 0));

            totalPrice += horizontalUProfileLm * parseFloat(gamma_pricing.price_u_horizontal_panel || 0);
            //console.log('horizontalUProfileLm:', horizontalUProfileLm, 'Price U Horizontal Panel:', gamma_pricing.price_u_horizontal_panel, 'Subtotal:', horizontalUProfileLm * parseFloat(gamma_pricing.price_u_horizontal_panel || 0));

            totalPrice += reinforcingProfileLm * parseFloat(gamma_pricing.price_reinforcing_profile || 0);
            //console.log('reinforcingProfileLm:', reinforcingProfileLm, 'Price Reinforcing Profile:', gamma_pricing.price_reinforcing_profile, 'Subtotal:', reinforcingProfileLm * parseFloat(gamma_pricing.price_reinforcing_profile || 0));

            totalPrice += rivetsPcs * parseFloat(gamma_pricing.price_rivets || 0);
            //console.log('rivetsPcs:', rivetsPcs, 'Price Rivets:', gamma_pricing.price_rivets, 'Subtotal:', rivetsPcs * parseFloat(gamma_pricing.price_rivets || 0));

            totalPrice += selfTappingScrewPcs * parseFloat(gamma_pricing.price_self_tapping_screw || 0);
            //console.log('selfTappingScrewPcs:', selfTappingScrewPcs, 'Price Self Tapping Screw:', gamma_pricing.price_self_tapping_screw, 'Subtotal:', selfTappingScrewPcs * parseFloat(gamma_pricing.price_self_tapping_screw || 0));

            totalPrice += dowelsPcs * parseFloat(gamma_pricing.price_dowels || 0);
            //console.log('dowelsPcs:', dowelsPcs, 'Price Dowels:', gamma_pricing.price_dowels, 'Subtotal:', dowelsPcs * parseFloat(gamma_pricing.price_dowels || 0));

            totalPrice += cornerPcs * parseFloat(gamma_pricing.price_corners || 0);
            //console.log('cornerPcs:', cornerPcs, 'Price Corners:', gamma_pricing.price_corners, 'Subtotal:', cornerPcs * parseFloat(gamma_pricing.price_corners || 0));
            ...
            $('#gamma_panel_width').val(formattedWidth);  // Will be like "1.80"
            $('#gamma_panel_height').val(formattedHeight); // Will be like "1.245"
            ...
            // Debug log
            console.log('Calculator Values Updated gamma:', {
                ...
                hiddenFields: {
                    ...
                    gamma_panel_width: $('#gamma_panel_width').val(),
                    gamma_panel_height: $('#gamma_panel_height').val(),
                    gamma_number_of_panels: $('#gamma_number_of_panels').val()
                }
            });
            ...
            $('#gamma-final-price').html(`<p>Крайна цена: ${totalPrice.toFixed(2)} лв.</p>`);
