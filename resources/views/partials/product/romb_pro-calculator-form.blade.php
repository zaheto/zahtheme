@php
    ...
    $panel_heights  = $romb_proData['panel_height'] ?? [];
    $default_height = $predefined_sizes[0]['height'] ?? '';
@endphp

<section class="fences-product">
    <div id="romb-pro-calculator-form" class="romb-pro-calculator-section">
        <h2 class="mb-2">Въведете вашите индивидуални размери на оградата:</h2>
        <form id="romb-pro-fence-calculator" class="romb-pro-calculator-section">
            <div class="form-row">
                <label for="romb-pro-panel-width" class="font-normal">Ширина на паното (m)</label>
                <input
                    class="border border-black/10 rounded-md px-4 py-2"
                    type="number"
                    id="romb-pro-panel-width"
                    name="romb-pro-panel-width"
                    step="0.01"
                    min="{{ $romb_proData['width_min'] }}"
                    max="{{ $romb_proData['width_max'] }}"
                    value="1.8"
                    required
                >
            </div>
            ...
            <div class="form-row">
                <label for="romb-pro-panel-height" class="font-normal">Височина на паното (m):</label>
                <select id="romb-pro-panel-height" name="romb-pro-panel-height" required>
            ...
            <div class="form-row">
                <label for="romb-pro-number-of-panels" class="font-normal">Брой пана (бр.)</label>
                <input
                    class="border border-black/10 rounded-md px-4 py-2"
                    type="number"
                    id="romb-pro-number-of-panels"
                    name="romb-pro-number-of-panels"
                    min="1"
                    value="1"
                    required
                >
        ...
        <script>
            jQuery(document).ready(function($) {
                ...
                $('#romb-pro-panel-width').val(firstSize.data('l'));
                $('#romb-pro-panel-height').val(firstSize.data('h'));

                $('.main-product-sizes__item').on('click', function(e) {
                    e.preventDefault();
                    $('#romb-pro-panel-width').val($(this).data('l'));
                    $('#romb-pro-panel-height').val($(this).data('h'));
