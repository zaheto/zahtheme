@php
    ...
    $panel_heights  = $romb_miniData['panel_height'] ?? [];
    $default_height = $predefined_sizes[0]['height'] ?? '';
@endphp

<section class="fences-product">
    <div id="romb-mini-calculator-form" class="romb-mini-calculator-section">
        <h2 class="mb-2">Въведете вашите индивидуални размери на оградата:</h2>
        <form id="romb-mini-fence-calculator" class="romb-mini-calculator-section">
            <div class="form-row">
                <label for="romb-mini-panel-width" class="font-normal">Ширина на паното (m)</label>
                <input
                    class="border border-black/10 rounded-md px-4 py-2"
                    type="number"
                    id="romb-mini-panel-width"
                    name="romb-mini-panel-width"
                    step="0.01"
                    min="{{ $romb_miniData['width_min'] }}"
                    max="{{ $romb_miniData['width_max'] }}"
                    value="1.8"
                    required
                >
            </div>
            ...
            <div class="form-row">
                <label for="romb-mini-panel-height" class="font-normal">Височина на паното (m):</label>
                <select id="romb-mini-panel-height" name="romb-mini-panel-height" required>
            ...
            <div class="form-row">
                <label for="romb-mini-number-of-panels" class="font-normal">Брой пана (бр.)</label>
                <input
                    class="border border-black/10 rounded-md px-4 py-2"
                    type="number"
                    id="romb-mini-number-of-panels"
                    name="romb-mini-number-of-panels"
                    min="1"
                    value="1"
                    required
                >
        ...
        <script>
            jQuery(document).ready(function($) {
                ...
                $('#romb-mini-panel-width').val(firstSize.data('l'));
                $('#romb-mini-panel-height').val(firstSize.data('h'));

                $('.main-product-sizes__item').on('click', function(e) {
                    e.preventDefault();
                    $('#romb-mini-panel-width').val($(this).data('l'));
                    $('#romb-mini-panel-height').val($(this).data('h'));
