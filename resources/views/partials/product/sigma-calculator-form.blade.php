@php
            ...
            $panel_heights = $sigmaData['panel_height'] ?? [];
...
<section class="fences-product">
    <div id="sigma-calculator-form" class="sigma-calculator-section">
        <h2 class="mb-2">SIGMA Въведете вашите индивидуални размери на оградата:</h2>
        <form id="sigma-fence-calculator" class="sigma-calculator-section">
            <div class="form-row">
                <label for="sigma-panel-width" class="font-normal">Ширина на паното (m)</label>
                <input 
                class="border border-black/10 rounded-md px-4 py-2"
                    ...
                    step="0.01" 
                    min="{{ $sigmaData['width_min'] }}" 
                    max="{{ $sigmaData['width_max'] }}" 
                    value="1.8"
                    required
                >
            </div>
            ...
            <div class="form-row">
                <label for="sigma-panel-height" class="font-normal">Височина на паното (m):</label>
                <select id="sigma-panel-height" name="sigma-panel-height" required>
                    @foreach ($sigmaData['panel_height'] as $height)
                        @php
                            $height = trim($height);
            ...
            <div class="form-row">
                <label for="sigma-number-of-panels" class="font-normal"> Брой пана (бр.) </label>
                <input 
                class="border border-black/10 rounded-md px-4 py-2"
                    type="number" 
    ...
    <script>
        jQuery(document).ready(function($) {
            ...
            $('#sigma-panel-width').val(defaultWidth);
            $('#sigma-panel-height').val(defaultHeight);
            ...
            $('.main-product-sizes__item').on('click', function(e) {
                ...
                // Update form inputs
                $('#sigma-panel-width').val(width);
                $('#sigma-panel-height').val(height);
