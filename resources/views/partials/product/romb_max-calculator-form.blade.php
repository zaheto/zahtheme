@php
    ...
    $panel_heights  = $romb_maxData['panel_height'] ?? [];
    $default_height = $predefined_sizes[0]['height'] ?? '';
@endphp

<section class="fences-product">
    <div id="romb-max-calculator-form" class="romb-max-calculator-section">
        <h2 class="mb-2">Въведете вашите индивидуални размери на оградата:</h2>
        <form id="romb-max-fence-calculator" class="romb-max-calculator-section">
            <div class="form-row">
                <label for="romb-max-panel-width" class="font-normal">Ширина на паното (m)</label>
                <input
                    class="border border-black/10 rounded-md px-4 py-2"
                    type="number"
                    id="romb-max-panel-width"
                    name="romb-max-panel-width"
                    step="0.01"
                    min="{{ $romb_maxData['width_min'] }}"
                    max="{{ $romb_maxData['width_max'] }}"
                    value="1.8"
                    required
                >
            </div>
            ...
            <div class="form-row">
                <label for="romb-max-panel-height" class="font-normal">Височина на паното (m):</label>
                <select id="romb-max-panel-height" name="romb-max-panel-height" required>
            ...
            <div class="form-row">
                <label for="romb-max-number-of-panels" class="font-normal">Брой пана (бр.)</label>
                <input
                    class="border border-black/10 rounded-md px-4 py-2"
                    type="number"
                    id="romb-max-number-of-panels"
                    name="romb-max-number-of-panels"
                    min="1"
                    value="1"
                    required
                >
        ...
        <script>
            jQuery(document).ready(function($) {
                ...
                $('#romb-max-panel-width').val(firstSize.data('l'));
                $('#romb-max-panel-height').val(firstSize.data('h'));

                $('.main-product-sizes__item').on('click', function(e) {
                    e.preventDefault();
                    $('#romb-max-panel-width').val($(this).data('l'));
                    $('#romb-max-panel-height').val($(this).data('h'));
